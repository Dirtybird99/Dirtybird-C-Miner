...
The repository includes the required `libsais` source under `extern/libsais` and bundled Astro SPSA assets under `lib/astrospsa`, so standalone source builds do not depend on a sibling checkout or a live GitLab fetch during configure.
