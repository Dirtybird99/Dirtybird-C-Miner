...
bool NUMAOptimizer::initialize() {
    ...
    // Silent NUMA initialization - no verbose output
...
void NUMAOptimizer::optimizeMemoryForMining(void* ptr, size_t size) {
    ...
    // For AstroBWT mining: RANDOM access pattern (SA construction, branch loop)
    // FIXED: Was MADV_SEQUENTIAL which caused cache pollution
...
void NUMAOptimizer::printAllocInfo(const DirtybirdAllocInfo & info) {
    const char* page_type_str;
    switch (info.page_type) {
      case DIRTYBIRD_PAGE_1GB:    page_type_str = "1GB huge pages"; break;
      case DIRTYBIRD_PAGE_2MB:    page_type_str = "2MB huge pages"; break;
