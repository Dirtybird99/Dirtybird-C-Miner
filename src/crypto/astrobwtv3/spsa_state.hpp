...
/**
 * Compact-cache decode path:
 * Uses a prebuilt stamp->start_chunk array to reduce hot decode memory traffic.
 */
inline int32_t merge_entry_to_global_pos_fast_starts(
    uint32_t entry,
    const uint16_t* stamp_start_chunks
) {
    uint16_t stamp_id = (entry >> 16) & 0x01FF;
    uint16_t relative_pos = entry & 0xFFFF;

    int32_t stamp_result = static_cast<int32_t>(
        (static_cast<int32_t>(stamp_start_chunks[stamp_id]) << 8) + relative_pos
    );
    int32_t direct_result = static_cast<int32_t>(entry & 0x7FFFFFFF);
...
/**
 * Compact-cache decode path using precomputed stamp base offsets (start_chunk << 8).
 */
inline int32_t merge_entry_to_global_pos_fast_bases(
    uint32_t entry,
    const int32_t* stamp_base_offsets
) {
    uint16_t stamp_id = (entry >> 16) & 0x01FF;
    uint16_t relative_pos = entry & 0xFFFF;

    int32_t stamp_result = stamp_base_offsets[stamp_id] + static_cast<int32_t>(relative_pos);
    int32_t direct_result = static_cast<int32_t>(entry & 0x7FFFFFFF);
...
/**
 * Batch decode path backed by compact stamp starts cache.
 */
inline void merge_entries_to_global_pos_batch4_starts(
    const uint32_t* entries,
    const uint16_t* stamp_start_chunks,
    int32_t* out_positions
) {
    ...
    int32_t sr0 = (static_cast<int32_t>(stamp_start_chunks[sid0]) << 8) + rp0;
    int32_t sr1 = (static_cast<int32_t>(stamp_start_chunks[sid1]) << 8) + rp1;
    int32_t sr2 = (static_cast<int32_t>(stamp_start_chunks[sid2]) << 8) + rp2;
    int32_t sr3 = (static_cast<int32_t>(stamp_start_chunks[sid3]) << 8) + rp3;
...
/**
 * Batch decode path backed by compact stamp base offsets cache.
 */
inline void merge_entries_to_global_pos_batch4_bases(
    const uint32_t* entries,
    const int32_t* stamp_base_offsets,
    int32_t* out_positions
) {
    ...
    int32_t sr0 = stamp_base_offsets[sid0] + static_cast<int32_t>(rp0);
    int32_t sr1 = stamp_base_offsets[sid1] + static_cast<int32_t>(rp1);
    int32_t sr2 = stamp_base_offsets[sid2] + static_cast<int32_t>(rp2);
    int32_t sr3 = stamp_base_offsets[sid3] + static_cast<int32_t>(rp3);
...
/**
 * Batch decode specialized for stamp-only entries (high bit already known set).
 */
inline void merge_entries_to_global_pos_batch4_stamp_only_starts(
    const uint32_t* entries,
    const uint16_t* stamp_start_chunks,
    int32_t* out_positions
) {
    ...
    out_positions[0] = (static_cast<int32_t>(stamp_start_chunks[sid0]) << 8) + rp0;
    out_positions[1] = (static_cast<int32_t>(stamp_start_chunks[sid1]) << 8) + rp1;
    out_positions[2] = (static_cast<int32_t>(stamp_start_chunks[sid2]) << 8) + rp2;
    out_positions[3] = (static_cast<int32_t>(stamp_start_chunks[sid3]) << 8) + rp3;
}

/**
 * Batch decode specialized for stamp-only entries using precomputed bases.
 */
inline void merge_entries_to_global_pos_batch4_stamp_only_bases(
    const uint32_t* entries,
    const int32_t* stamp_base_offsets,
    int32_t* out_positions
) {
    ...
    out_positions[0] = stamp_base_offsets[sid0] + static_cast<int32_t>(rp0);
    out_positions[1] = stamp_base_offsets[sid1] + static_cast<int32_t>(rp1);
    out_positions[2] = stamp_base_offsets[sid2] + static_cast<int32_t>(rp2);
    out_positions[3] = stamp_base_offsets[sid3] + static_cast<int32_t>(rp3);
}
