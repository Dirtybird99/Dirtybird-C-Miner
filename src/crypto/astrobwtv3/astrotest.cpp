...
#if defined(__AVX2__)
  constexpr bool kPrintAvx2MaskDebug = false;
  if (kPrintAvx2MaskDebug) {
    int scale = 32 - 18;
...
namespace {
constexpr bool kPrintRepeatVectorDebug = false;
}
...
int runDeroOpTests(int testOp, int dataLen) {
  ...
  workerData *controlWorker = (workerData*)malloc_huge_pages(sizeof(workerData));
  if (!controlWorker) {
    fprintf(stderr, "Failed to allocate controlWorker\n");
    return -1;
  }
  initWorker(*controlWorker);
  lookupGen(*controlWorker, nullptr, nullptr);
  ...
  if (!testWorker) {
    fprintf(stderr, "Failed to allocate testWorker\n");
    free_huge_pages(controlWorker);
    return -1;
  }
  ...
  if (!z_testWorker) {
    fprintf(stderr, "Failed to allocate z_testWorker\n");
    free_huge_pages(controlWorker);
    free_huge_pages(testWorker);
    return -1;
  }
...
int TestAstroBWTv3(bool quiet=false)
{
  ...
  if (!worker) {
    fprintf(stderr, "Failed to allocate workerData in TestAstroBWTv3\n");
    return -1;
  }
  initWorker(*worker);
  lookupGen(*worker, nullptr, nullptr);
  initWolfLUT();  // CRITICAL: Initialize CodeLUT_16 for AVX2 wolfPermute path
  ...
  if(!quiet && kPrintRepeatVectorDebug) {
...
int TestAstroBWTv3repeattest(bool quiet)
{
  ...
  workerData *worker = (workerData *)malloc_huge_pages(sizeof(workerData));
  if (!worker) {
    fprintf(stderr, "Failed to allocate workerData in TestAstroBWTv3repeattest\n");
    return -1;
  }
  initWorker(*worker);
  lookupGen(*worker, nullptr, nullptr);
