...
#define DIRTYBIRD_TARGETS_X86_AVX512_WOLF "avx512f,avx512bw,avx512vl", DIRTYBIRD_FEATURES_ZNVER4, DIRTYBIRD_FEATURES_ZNVER5
...
// DERO Miner: Initialize lazily to avoid static initialization order fiasco
alignas(32) uint16_t *CodeLUT_16 = nullptr;

// Forward declaration
void initWolfPermuteDispatch();

void initWolfLUT() {
  // Allocate CodeLUT_16 here instead of at static initialization time
  // to avoid issues with malloc_huge_pages accessing uninitialized globals
  if (CodeLUT_16 == nullptr) {
    CodeLUT_16 = (uint16_t*)malloc_huge_pages(257*4);
    if (CodeLUT_16 == nullptr) {
      // Fallback to regular allocation if huge pages fail
      CodeLUT_16 = (uint16_t*)malloc(257*4);
    }
  }
  ...
  // Initialize wolfPermute dispatch function pointer
...
#define WOLF_BRANCH_AVX512(IN, POS2VAL, OPCODE)                                 \
  ...
  for (int i = 3; i >= 0; --i) {                                                \
    uint8_t insn = ((OPCODE) >> (i << 2)) & 0xF;                                \
    switch (insn) {                                                             \
      case 0: (IN) = _mm512_add_epi8((IN), (IN)); break;                        \
      case 1: (IN) = _mm512_sub_epi8((IN), _mm512_xor_si512((IN), _mm512_set1_epi8(97))); break; \
      case 2: (IN) = _mm512_mul_epi8((IN), (IN)); break;                        \
      case 3: (IN) = _mm512_xor_si512((IN), _mm512_set1_epi8((POS2VAL))); break; \
      case 4: (IN) = _mm512_xor_si512((IN), _mm512_set1_epi64(-1LL)); break;    \
      case 5: (IN) = _mm512_and_si512((IN), _mm512_set1_epi8((POS2VAL))); break;\
      case 6: (IN) = _mm512_sllv_epi8((IN), _mm512_and_si512((IN), vec_3_512)); break;\
      case 7: (IN) = _mm512_srlv_epi8((IN), _mm512_and_si512((IN), vec_3_512)); break;\
      case 8: (IN) = _mm512_reverse_epi8((IN)); break;                          \
      case 9: (IN) = _mm512_xor_si512((IN), popcnt512_epi8((IN))); break;       \
      case 10: (IN) = _mm512_rolv_epi8((IN), (IN)); break;                      \
      case 11: (IN) = _mm512_rol_epi8((IN), 1); break;                          \
      case 12: (IN) = _mm512_xor_si512((IN), _mm512_rol_epi8((IN), 2)); break;  \
      case 13: (IN) = _mm512_rol_epi8((IN), 3); break;                          \
      case 14: (IN) = _mm512_xor_si512((IN), _mm512_rol_epi8((IN), 4)); break;  \
...
// Main wolfPermute with AVX2 dispatch
void wolfPermute(uint8_t *in, uint8_t *out, uint16_t op, uint8_t pos1, uint8_t pos2, workerData &worker)
{
#if defined(__x86_64__) || defined(_M_X64)
  // Fast path: use AVX2 when available (handles up to 32 bytes at once)
  if (g_has_avx2 && pos2 > pos1) {
    // AVX2 path processes 32 bytes at a time with masking
    branch_tables::wolf_permute_avx2(in, out, static_cast<uint8_t>(op), pos1, pos2);
    return;
  }
#endif
...
// Initialize dispatch - detect AVX2 support
void initWolfPermuteDispatch() {
#if defined(__x86_64__) || defined(_M_X64)
...
// Explicit AVX2 version (calls through main wolfPermute which dispatches)
...
#endif
    wolfPermute(in, out, op, pos1, pos2, worker);
}

// AVX512 version (falls back to AVX2 for now - can be enhanced later)
